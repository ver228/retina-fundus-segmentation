...
def unet_constructor(n_inputs,
    ...
    if OutputBlock is None:
        output_block = nn.Sequential(
                nn.Conv2d(initial_filter_size, n_outputs, kernel_size = 3, padding = 1),
                nn.Sigmoid()
                )
    else:
...
class DiceCoeff(nn.Module):
    ...
    def forward(self, x_in, target):
        ...
        eps = 0.0001
        self.inter = torch.dot(x_in.reshape(-1), target.reshape(-1))
        self.union = torch.sum(x_in) + torch.sum(target) + eps
...
if __name__ == '__main__':
    ...
    model = unet_constructor(n_in, n_out, batchnorm = batchnorm)
    xout = model(X)
...
#        print(f.shape)
    ...
    criterion = DiceCoeff()
    loss = criterion(xout, target)#(xout-target).abs().sum()
    loss.backward()
